package com.atguigu.sparkmall2019.mock

import java.text.SimpleDateFormat
import java.util.UUID

import com.atguigu.sparkmall2019.common.model.{CityInfo, ProductInfo, UserInfo, UserVisitAction}
import com.atguigu.sparkmall2019.common.util.ConfigurationUtil
import com.atguigu.sparkmall2019.mock.util.{RanOpt, RandomDate, RandomNum, RandomOptions}
...
object MockerOffline {
  val userNum = 100; //用户数
  val productNum = 100  //产品数量
  val sessionNum = 10000  //session

  val pageNum = 50  //页面总数
  val cargoryNum = 20  //商品类别
  ...
  def mockUserAction() = {
    ...
    val randomDate = RandomDate(startDate, endDate, logAboutNum)// 随机生成一个时间段的时间序列
    for (i <- 1 to sessionNum) {
      ...
      while (!isQuit) {
        ...
        if (action == "quit") {
          ...
          val actionDateTime = randomDate.getRandomDate()//得到date
          ...
          action match {
            ...
            case "click" => clickCategoryId = RandomNum(1, cargoryNum) //得到随机类别
              clickProductId = RandomNum(1, productNum)//得到随机商品id
            case "order" => orderCategoryIds = RandomNum.multi(1, cargoryNum, RandomNum(1, 5), ",", false)
              orderProductIds = RandomNum.multi(1, cargoryNum, RandomNum(1, 5), ",", false)    //2,3,5,7,1
